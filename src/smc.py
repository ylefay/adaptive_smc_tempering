...
class GenericAdaptiveWasteFreeTemperingSMC:
    ...
    def __init__(self, logbase_density_fn: Callable[[ArrayLike], ArrayLike],
                 ...
                 build_RWMH_log_proposals: Callable[
                     [ArrayLike, ArrayLike, int], Callable[[ArrayLike, ArrayLike], ArrayLike]],
                 build_RWMH_proposal_samplers: Callable[
                     [ArrayLike, ArrayLike, int], Callable[[PRNGKey, ArrayLike], ArrayLike]]
                 ) -> None:
        ...
        self.build_RWMH_log_proposals = build_RWMH_log_proposals
        self.build_RWMH_proposal_samplers = build_RWMH_proposal_samplers
    ...
    def sample(self, key: PRNGKey, num_parallel_chain: int, num_mcmc_steps: int,
        ...
        init_proposed_particles = jax.vmap(
            self.build_RWMH_proposal_samplers(initial_rwmh_proposal_parameter, particles, 0), in_axes=(0, 0))(subkeys,
                                                                                                           init_particles)
        ...
        def body_fn(i, carry):
            ...
            def inside_body_fn(key, particle):
                ...
                def inside_inside_body_fn(p, carry):
                    ...
                    new_proposed_particle = self.build_RWMH_proposal_samplers(rwmh_proposal_parameters.at[i].get(),
                                                                              particles, i)(
                        subkey, particle)  # to fix
                    ...
                    new_log_q = self.build_RWMH_log_proposals(rwmh_proposal_parameters.at[i].get(), particles, i)(
                        particle, new_proposed_particle)  # to fix
                    ...
                    accept_MH_boolean = accept_reject_MH_step(key,
                                                              ...
                                                              self.build_RWMH_log_proposals(
                                                                  rwmh_proposal_parameters.at[i].get(), particles, i)(
                                                                  new_proposed_particle, particle
                                                              )
                                                              , new_log_q
                                                              )
            ...
            new_log_weights = jax.vmap(self.log_weights_fn(diff_tempering_sequence.at[i].get()))(
                new_particles.reshape(num_particles, *shape)).reshape((num_parallel_chain, num_mcmc_steps +1 ))  # to fix
            ...
            def estimate_of_esjd(param):
                _new_particles = new_particles.reshape((num_particles, *shape))
                _new_proposed_particles = new_proposed_particles.reshape((num_particles, *shape))
                log_proposal_fn = jax.vmap(self.build_RWMH_log_proposals(param, particles, i))  # to fix
                ...
                ratio_proposal = jnp.exp(diff_log_proposal)
                to_sum = (new_weights * new_d * jax.lax.max(1., new_g * ratio_proposal)).reshape(-1) * jnp.exp(
                    one_term_log_proposal - new_log_q)
