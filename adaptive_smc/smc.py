...
class GenericAdaptiveWasteFreeTemperingSMC:
    ...
    def __init__(self, logbase_density_fn: Callable[[ArrayLike], ArrayLike],
        ...
        self.vmapped_log_likelihood_fn = jnp.vectorize(log_likelihood_fn, signature='(d)->()') #Assuming a vmappable function
...
class GenericAdaptiveWasteFreeTemperingSMCSaveMemory:
    """
    A class that implements an adaptive Tempering SMC algorithm with waste-free SMC using parametric random-walk Metropolis-Hastings proposals.
    """
    ...
    def sample(self, key: PRNGKey, num_parallel_chain: int, num_mcmc_steps: int,
        ...
        def make_inner_loop(i: int, prev_lmbda: float, mh_proposal_parameter: ArrayLike,
            ...
            def inside_body_fn(key, particle):
                ...
                def inside_inside_body_fn(p, carry):
                    key, proposed_particles_across_p, particles_across_p, log_g_across_p, d_across_p, log_q_across_p, = carry
                    particle = particles_across_p.at[p - 1].get()
                    subkey_p = jax.random.fold_in(key, p)
                    _, key = jax.random.split(key)
                    ...
                    particles_across_p = particles_across_p.at[p].set(new_particle)
                    return key, proposed_particles_across_p, particles_across_p, log_g_across_p, d_across_p, log_q_across_p

                _, _proposed_particles, _particles, _log_g, _d, _log_q = jax.lax.fori_loop(1,
                                                                                                              P,
                                                                                                              inside_inside_body_fn,
                                                                                                              (
                                                                                                                  key,
                ...
                return _particles, _proposed_particles, _log_g, _d, _log_q

            return inside_body_fn

        def body_fn(i, carry):
            particles, log_weights, mh_proposal_parameters, criteria, tempering_sequence, diff_tempering_sequence, log_normalizations = carry
            subkey = jax.random.fold_in(key, i)
            ancestors = multinomial(subkey, jnp.exp(log_weights.at[i - 1].get().reshape(-1)), num_parallel_chain)
            resampled_particles = particles.at[i - 1].get().reshape((num_particles, dim)).at[ancestors].get()
            ...
            keys = jax.random.split(subkey, num_parallel_chain)
            new_particles, new_proposed_particles, new_log_g, new_d, new_log_q = inside_body_fn(
                keys, resampled_particles)
            ###
            ...
            mh_proposal_parameters = mh_proposal_parameters.at[i].set(new_mh_proposal_parameter)
            return particles, mh_proposal_parameters, tempering_sequence, diff_tempering_sequence, log_normalizations

        particles, log_weights, mh_proposal_parameters, tempering_sequence, diff_tempering_sequence, log_normalizations = jax.lax.fori_loop(
            1, iteration + 1,
            body_fn, (
                particles,
                log_weights,
        ...
        return particles, log_weights, mh_proposal_parameters, tempering_sequence, log_normalizations
