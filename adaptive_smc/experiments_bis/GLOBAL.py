...
def default_config():
    ...
    all_keys = jax.vmap(lambda key: jax.random.split(key, parallel_repetitions))(seq_keys)

    prefix = "default_config"

    config = {'dim': dim, 'num_parallel_chain': num_parallel_chain, 'num_mcmc_steps': num_mcmc_steps,
              'parallel_repetitions': parallel_repetitions, 'target_ess': target_ess, 'OUTPUT_PATH': OUTPUT_PATH,
