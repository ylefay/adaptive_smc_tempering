...
from build.lib.adaptive_smc.metropolis import accept_reject_mh_step
...
class GenericAdaptiveWasteFreeTemperingSMC:
    ...
    def estimate_expectation_criteria_fun(self, state, i):
        ...
        tempering_sequence = state.tempering_sequence
        proposed_particles = state.particles.at[i].get()
        ...
        def fun(param):
            ...
            log_weights_proposal = log_my_new_proposal(particles, proposed_particles)
            weights, _ = normalize_log_weights(log_weights + log_weights_proposal - log_weights_current_proposal)

            return jnp.sum(weights * g)
    ...
    def sample(self, key: PRNGKey, num_parallel_chain: int, num_mcmc_steps: int,
        ...
        log_normalizations = jnp.zeros((iteration, num_parallel_chain, P))
        ...
        new_mh_proposal_parameter = self.optimisation(
            to_optimize,
            mh_proposal_parameters.at[0].get()
        )
        ...
        def make_inner_loop(i: int, couple_particles: ArrayLike,
            ...
            def inside_body_fn(key, couple_particle):
                ...
                def insinde_inside_body_fn(p, carry):
                    key, couple_particles_across_p = carry
                    ...
                    accept_MH_boolean = accept_reject_mh_step(
                        key,
                    ...
                    couple_particles_across_p = couple_particles_across_p.at[p].set(new_proposed_particle)
                    return key, couple_particles_across_p

                _, _couple_particles = jax.lax.fori_loop(
                    1,
                    P,
        ...
        def body_fn(i, carry):
            ...
            new_couple_particles = inside_body_fn(keys, resampled_couple_particles)
            ...
            to_optimize = self.estimate_expectation_criteria_fun(state, i)
            new_mh_proposal_parameter = self.optimisation(
                to_optimize,
                mh_proposal_parameters.at[i].get()
            )
            criteria = criteria.at[i - 1].set(jax.vmap(to_optimize)(GRID_CRITERIA))
